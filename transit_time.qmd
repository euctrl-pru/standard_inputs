...
The transit time reflects the **average flying time of an aircraft in the airspace of the ANSP over a year**.

@tbl-transit-time provides an overview of the average transit time (expressed 
in minutes) per ANSP in 2019.

The data used to build this table can be accessed on [AIU portal](https://ansperformance.eu/data/), 
where data for more recent years cal also be found.
...
read_xlsx("data/transit_time.xlsx") %>%
  ...
  tab_footnote(
    footnote = "The Maastricht Upper Area Control Centre (MUAC) manages the upper airspace (from 24,500 to 66,000 feet) over Belgium, the Netherlands, Luxembourg and north-west Germany.",
...
```
*Source: EUROCONTROL Aviation Intelligence Unit[@aiuportal]*
...
![Average transit time per country in 2019. *Source*: EUROCONTROL](figures/transit_time.svg){#fig-transit-time fig-alt="A drawing of transit times."}
