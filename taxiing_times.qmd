...
based on flights in ECAC area.

```{r}
#| label: tbl-taxi-time
#| tbl-cap: Average taxi time duration

read_xlsx("data/taxi_time.xlsx",
          sheet = "total") %>% 
  gt() %>% 
  cols_label(
    year = "Year",
  ...
  tab_source_note(
    source_note = md("*Source: EUROCONTROL Central Office for Delay Analysis (CODA)*")
  )
```
...
taxi-in and taxi-out durations are calculated on the basis of data sent 
by airlines to CODA.

The table below shows the average additional taxi out time per departure in top 
30 airports in terms of movements, excluding Turkish airports.

```{r}
#| label: tbl-taxi-time-additional
#| tbl-cap: Average additional taxi time per departure
...
Values in @tbl-taxi-time-additional are based on actual data from airports. The 
additional taxi-out time is a proxy for the average departure runway queuing 
time on the outbound traffic flow during congestion periods at airports. It is 
...
based on taxi-out times in periods of low traffic demand. There is one 
unimpeded time per stand runway combination at each airport.

## Other possible sources

[EUROCONTROL CODA Taxi times - Summer and Winter reports](https://www.eurocontrol.int/search?keywords=Taxi+times&sort_by=search_api_relevance)
This source shows, by airport, seasonal taxi time statistics for the IATA 
winter season and the IATA summer season:
...
Furthermore for additional granularity, taxi-out times by wake turbulence 
category are also offered for a number of airports.
