...
:   
    ...
    read_csv("data/rate-of-fuel-burn.csv") %>% 
      gt() %>% 
    tab_header(title = md("Fuel burn rates (kg/minutes) in flight phases
                          where delay can occur for most representative
                          aircraft types flying (with restriction in use)")) %>%
    tab_footnote(
      footnote = "It is strictly prohibited to use the fuel burn values to
                  compare fuel efficiency and emission data between aircraft
                  types from the same or different manufactures.
                  Data are provided here to allow the drafting of aggregate
                  business cases.",
      locations = cells_title(groups = c("title"))) %>%
    tab_footnote(
      footnote = "The useful load is the sum of the payload and the fuel.
...
[Amount of emissions released by fuel burn](#amount-of-emissions-released-by-fuel-burn),
[cost of emissions] and
