...
the end of 2024. Please note there might be some marginal differences with the
...
@tbl-cns-ecac-surv shows the number of Surveillance units in ECAC at the end of 2024.

```{r}
#| label: tbl-cns-ecac-surv
#| tbl-cap: Surveillance units in EUROCONTROL Member States in 2024
...
read_xlsx(here::here("data", "cns_infrastructure.xlsx"),
  ...
  tab_footnote(
    footnote = "Please note that the values marked with an asterisk represent confidential information. For possible disclosure of this information, please contact the state POC in LSSIP",
    locations = cells_body(
      columns = 1,
      rows = c(3,4,7,8,10,11,12,15,17,20,21,25,28,29,31,32,35,36,37,40)
    )
    )%>% 
   tab_footnote(
  ...
  tab_source_note (
    source = md("*Source: EUROCONTROL Surveillance Unit Surveillance Database; LSSIP+ Cycle 2024 – SUR Questionnaire*")
  )
```
...
According to the most recent figures, in Europe there are 581 Mode-S radars, 
148 PSRs and 117 SSRs, either combined or standalone. As the allocation and 
