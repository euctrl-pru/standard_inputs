...
they would accept, directly or indirectly, for time lost.
...
read_xlsx(here::here("data", "passenger_time_value.xlsx"),
  ...
  tab_footnote(
  footnote = "Values adjusted from 2002 to 2025 prices based on inflation",
...
read_xlsx(here::here("data", "passenger_time_value.xlsx"),
  ...
  tab_footnote(
  footnote = "Values adjusted from 2015 to 2025 prices based on inflation",
locations = cells_column_labels(
  columns = value
)
)
...
read_xlsx(here::here("data", "passenger_time_value.xlsx"),
  ...
  tab_footnote(
  footnote = "Values adjusted from 2014 to 2025 prices based on inflation and using the exchange rate in general parameters",
locations = cells_column_labels(
  columns = value
)
) 
...
@tbl-pas-time-value-eu, @tbl-pas-time-value-fr and @tbl-pas-time-value-uk present
a collection of different values that can be used for this purpose. It is to be 
noted that, in this section, the value of time is not cited as a function of delay 
duration. This is an important consideration when using the value, since the longer 
the delay duration, the higher the value.
...
**€159 per hour.** This value was provided to EUROCONTROL by airline members of 
the SESAR CBA team in 2011 and is, therefore, adjusted to 2025 prices based on inflation. 
...
read_xlsx(here::here("data", "passenger_time_value.xlsx"),
  ...
  tab_footnote(
  footnote = "Values adjusted from 2015 to 2025 prices based on inflation and using the exchange rate in general parameters",
locations = cells_column_labels(
  columns = value
)
) 
...
to inflation from 2015 to 2025 prices.
