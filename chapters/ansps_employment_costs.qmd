...
#| tbl-cap: Average yearly ANSP employment costs (2024 prices)
...
read_xlsx(here("data", "ansp_employment.xlsx"),
  ...
  tab_source_note(
    source = md("*Source: [EUROCONTROL PRU, ‘ATM Cost-Effectiveness (ACE) Benchmarking Report (2026 Edition)’](https://ansperformance.eu/publications/prc/ace/)*")
  ) %>% 
  tab_options(
    table.width = pct(100)
   ) %>% 
...
#| tbl-cap: Individual ANSP and average European system FTE costs in thousand euros (2024 prices)
...
to gate-to-gate cost (i.e. en-route and terminal costs) and are expressed in 2024 prices.
